// ═══════════════════════════════════════════════════════════════
// FILTRO V6 - MULTILINGÜE + CORREGIDO PARA TU WORKFLOW
// Correcciones: Nombre correcto del Webhook + Detección de idioma
// ═══════════════════════════════════════════════════════════════
...
// 2. OBTENER TU NÚMERO (Corregido: usa 'Webhook' no 'Webhook (WhatsApp)')
let rawRemoteJid = "";
try {
    // Opción A: Tu estructura exacta (body.data.key...)
    if ($('Webhook').item.json.body.data && $('Webhook').item.json.body.data.key) {
        rawRemoteJid = $('Webhook').item.json.body.data.key.remoteJid;
    }
    // Opción B: Estructuras antiguas (body.key...)
    else if ($('Webhook').item.json.body.key) {
        rawRemoteJid = $('Webhook').item.json.body.key.remoteJid;
    }
} catch (e) {
    console.log("⚠️ No se pudo leer el RemoteJid automáticamente");
}
...
function detectLanguage(text) {
    ...
    const patterns = {
        de: /\b(der|die|das|ist|sind|haben|können|werden|möchten|Guten|Sehr|Hotels?|euros?|preis)\b/i,
        en: /\b(the|is|are|have|can|will|would|Hello|Good|Hotel|Booking|price|euros?)\b/i,
        es: /\b(el|la|los|las|es|son|tiene|puede|quiere|Hola|Buenos|Hotel|Reserva|precio|euros?)\b/i,
        it: /\b(il|la|gli|le|è|sono|ha|può|vuole|Ciao|Buona|Hotel|Prenotazione|prezzo|euro)\b/i
    };
...
const googleVoices = {
    ...
    en: {
        languageCode: "en-US",
        name: "en-US-Neural2-F", // Voz femenina inglesa neural
        ssmlGender: "FEMALE"
    },
    es: {
        languageCode: "es-US",
        name: "es-US-Neural2-B", // Tu voz actual (masculina)
        ssmlGender: "MALE"
    },
    it: {
        languageCode: "it-IT",
...
return {
    json: {
        ...
        // CAMPOS PARA GOOGLE TTS MULTILINGÜE
