...
async function run() {
  ...
  try {
    const { data: logs, error } = await db.from('sales_outreach_log')
      .select('created_at, lead_id, message_preview, status, direction, channel')
    ...
    console.log(`Recent Inbound Messages (Total ${logs.length}):`);
    for (const log of logs) {
      ...
      console.log(`  Direction: ${log.direction} | Channel: ${log.channel}`);
